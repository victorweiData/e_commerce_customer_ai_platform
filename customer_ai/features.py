...
def build_sales_fact_with_churn() -> Path:
    ...
    pay_agg = payments.groupby("order_id", as_index=False).agg(
        total_paid=("payment_value", "sum"), n_payments=("payment_sequential", "count")
    )
    ...
    last_purch = (
        fact.groupby("customer_unique_id", as_index=False)["order_purchase_timestamp"]
        .max()
...
def build_order_features() -> Path:
    ...
    df = orders.merge(items, on="order_id", how="left").merge(
        products, on="product_id", how="left"
    )
    df["late_delivery"] = (df["days_diff_estimate"] < 0).astype(int)

    feats = df.groupby("order_id", as_index=False).agg(
...
def build_churn_features() -> Path:
    ...
    rfm = fact.groupby("customer_unique_id", as_index=False).agg(
    ...
    behav = fact.groupby("customer_unique_id", as_index=False).agg(
    ...
    cust_full = rfm.merge(behav, on="customer_unique_id").merge(
        churn, on="customer_unique_id"
    )
...
def build_segment_features() -> Path:
    ...
    cols = [
        ...
        if c not in {"customer_unique_id", "churn", "first_purchase"}
    ]
...
def build_segment_features_extended() -> Path:
    ...
    fact["state_grouped"] = np.where(
        fact["customer_state"].isin(top_states), fact["customer_state"], "OTHER"
    )
    geo = (
        pd.get_dummies(
    ...
    fact["is_promo"] = (
        fact["order_purchase_timestamp"].dt.month.isin(PROMO_MONTHS).astype(int)
    )
    promo = fact.groupby("customer_unique_id", as_index=False).agg(
    ...
    # assemble
    seg_ext = (
        cust_full.drop(columns=["first_purchase", "churn"])
